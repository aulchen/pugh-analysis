...
\begin{align*}
G_k &= \{x \in \R^n: xk \in \Z^n \}
\end{align*}

In other words, $G_k$ is $\Z^n$ shrunk by a factor of $k$, so all the points are $\frac{1}{k}$ distance apart from each other. Create a collection of open balls $A_k$ with radius $\frac{1}{k}$ and centers on the points of $G_k$, and define that collection as $\A_k$. Note that by the geometry, $\cup_{A_k \in \A_k} A_k = \R^n$

Let $X$ be a closed set in $\R^n$. For all $k$, let $C_k$ be the union of all open balls in $\A_k$ such that $A_k \cap X \neq \emptyset$. $A_k$ is the union of open sets and thus open. Since 
