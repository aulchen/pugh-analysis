...
Consider the function $f(x) = \sqrt{x}$, and take a Taylor approximation around $x = n$, where $n$ is a positive natural number. For all $x \in \R^+$, we have
...
TODO. YOU DON'T KNOW WHETHER THE FUNCTION IS ANALYTIC, AND EVEN IF IT IS, WHAT THE RADIUS OF CONVERGENCE IS.
