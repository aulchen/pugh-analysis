...
The textbook integration by parts formula assumes that $f$ and $g$ are continuous on $[a, b]$. In this case, the interval in question is $[1, 5]$. However, $f$ is discontinuous at $x=4$, and $g$ is discontinuous at $x = 2$.
...
However, in this case, $fg'$ is zero on $[1, 5]$, except at $x=2$, where it is undefined. A similar thing holds for $f'g$ at $x=4$. Since a potential antiderivative of $fg'$ has to have its derivative equal $fg'$ everywhere, not just almost everywhere, $fg'$ and $f'g$ do not have antiderivatives.
