...
Let $\psi$ be differentiable. We first divide critical points into two types. I shall define a \textbf{critical point of type 1} of a function $\psi$ as a critical point $x$ such that $\psi'(x)$ is uniformly zero in an open neighborhood of $x$. In other words, there exists a $\epsilon > 0$ such that for all $y$ such that $|x-y| < \epsilon$, $\psi'(y) = 0$. I will denote a \textbf{critical point of type 2} as all other critical points. It's clear that the disjoint union of these two types is the set of all critical points of $\psi$. I will define \textbf{critical values of type 1} to be critical values that correspond to critical points of type 1, and \textbf{critical values of type 2} similarly. It's clear that the disjoint union of these sets is the set of all critical values of $\psi$. I have no idea if this characterization is standard, but that's what I've come up with.

\textbf{REWRITE AND SIMPLIFY THIS LATER.}

The immediate characterization for roots of type 2 is stated below.
...
\begin{corollary}
Let $f$ be continuous, and let $x$ be a zero of type 2 of $f$. 
\end{corollary}

We next state a lemma on non-zero points of continuous functions implying an interval with no zeros. This can be thought of as non-zero points of continuous functions creating 'exclusion zones' with a delta-radius that contain no zeros.
...
We now introduce some useful terminology (that I have no idea whether is standard, but I am going to use it). Let $f: [a, b] \rightarrow \R$ be continuous, and let $x$ be a point such that $f(x) \neq 0$. The \textbf{covering interval of x} is the open interval between the nearest zero of $f$ to the left of $x$, and the nearest zero of $f$ to the right of $x$. This interval covers $x$. By Lemma \ref{NearestZeros}, this is a well-defined construction.
...
Note that covering intervals are disjoint, since any two covering intervals are separated by at least one zero of $f$.
...
\begin{proof}
Suppose not. Then $x$ is not the nearest zero of type 2 to any nonzero point of $f$. This means that there are areas where $f$ is constantly zero to the left and right of $x$ until $f$ reaches a zero of type 2, which implies that $x$ is a zero of type 1. This is a contradiction.
\end{proof}
\end{lemma}

\begin{corollary}
\label{ZeroType2Cardinality}
...
We can then consider the set of points where $\frac{1}{2} \geq f(x) < 1$, and define $A_2$ to be the set of non-duplicate covering intervals that cover those points. Then there exists $\delta_2 > 0$ such that the covering intervals in $A_2$ have length $\geq 2 \delta_2$, implying that there are at most $\frac{b-a}{2 \delta_2}$ intervals in $A_2$.

Continuing in this way for all $k \in \N$, we see that for all $k$, there are countably many covering intervals in $A_k$. Let $A$ be the union of all these sets, and thus the set of all covering intervals. It's clear that the intervals in $A$ cover all non-zero points of $f$, and that $A$ is countable. By Lemma \ref{ZeroType2Cardinality}, the set of zeros of type 2 is at most countable.
\end{proof}
\end{theorem}
...
\begin{theorem}
If $f: [a, b] \rightarrow \R$ is continuously differentiable, then $f$ has countably many zeros of type 1.
\begin{proof}
