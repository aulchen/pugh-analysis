...
(The hint in the book says that if I can prove that $B$ is closed and bounded with respect to the Euclidian metric, then $B$ is compact. I still need to figure out exactly why this is true)

Let $\|\cdot \|_E$ be the Euclidean norm on $\mathbb{R}^m$.

For boundedness, consider the unit circle in $\mathbb{R}^m$ under the standard Euclidean norm. This is a closed and bounded subset of $\mathbb{R}^m$, so it is a compact. Since all norms are  continuous, $\| \cdot \|: \mathbb{R}^m \rightarrow \mathbb{R}$ is a continuous function, and so the image of the unit circle under $\| \cdot \|$ obtains maximum and minimum values. The minimum value is trivially 0, because the zero-vector in $\mathbb{R}^m$ is an element of $B$, and all norms are positive definite.

Let $M$ be the maximum. If $M \geq 1$, then $B$ is a subset of the unit circle, and $B$ is bounded under the Euclidean metric. Otherwise, define $B' = \{x \in \mathbb{R}^m : \|x\| \leq M\}$. By construction, $B'$ is a subset of the unit disk, and is thus bounded under the Euclidean metric. Dilate $B'$ and the unit disk by a factor of $1/M$. By the norm property that $\|\alpha x \| = |\alpha| \|x\|$, after dilation, $B'$ maps to $B$, and the unit disk maps to a circle with radius 1/M centered at the origin. It's clear that $B$ is a subset of this disk, thus making $B$ bounded under the Euclidean metric.
