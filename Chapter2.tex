...
We first start with a theorem.

\begin{theorem}
The identity transformation from $(\mathbb{R}^m, \|\cdot\|_E)$ to $(\mathbb{R}^m, \|\cdot\|)$ is continuous.
\begin{proof}
By the translation invariance of norms, if we show that the identity transformation is continuous at the origin, we have shown that it is continuous everywhere.
Let $\epsilon > 0$, and let $B = \{x \in \mathbb{R}^m: \|x\| = \epsilon\}$ be the circle with radius epsilon under the $\|\cdot \|$ norm. By homogeneity, $B$ is a curve that does not intersect itself, and by positive definiteness, $B$ does not pass through the origin. Considering $B$ as a subset of $(\mathbb{R}^m, \|\cdot\|_E$, $B$ is closed since it's a curve.

(I'm not sure how to show boundedness. Ugh.)
\end{proof}
...
For closedness, norms are continuous functions from $\mathbb{R}^m$ to $\mathbb{R}$. It's clear that $B$ is the preimage of the closed set $[0, 1] \subset \mathbb{R}$, implying that $B$ is closed with respect to $\|\cdot\|$. Since the identity transformation from the Euclidean norm to the $\|\cdot\|$ norm is continuous, this implies that $B$ is closed with respect to the Euclidean norm.
