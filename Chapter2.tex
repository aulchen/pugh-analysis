...
If $M$ is empty then continuity is trivial. Otherwise, let $a_n \rightarrow p \in M$ be an arbitrary convergent sequence. Because $(a, f(a))_n$ is a sequence in $M \times f(M)$ which is compact, there is a convergent subsequence $(a, f(a))_{n_k} \rightarrow (q, r) \in M \times f(M)$. Looking at $a_n$ and $a_{n_k}$, $a_n \rightarrow p$ and $a_{n_k} \rightarrow q$ imply that $q = p$.

Because $f$ is a function, the only point in $M \times f(M)$ with first coordinate $p$ is $(p, f(p))$. Thus $(a, f(a))_{n_k} \rightarrow (p, f(p))$. Using the sum metric again, this implies that $f(p)_{n_k} \rightarrow f(p)$. If I can somehow show that the main sequence $f(a)_n$ itself has a limit, then its limit must be $f(p)$, but I don't know how to show that.
