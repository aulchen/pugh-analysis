...
\begin{theorem}
Let $p_0, p_1, p_2 \dots p_k \dots \in \R$ be the representations of the decimal subintervals $I_k$ length $10^{-k}$ that contain uncountable elements of $E$. Then there exists $p \in \cap_{k=0}^\infty I_k$ such that $p$ is a condensing point of $E$.
\begin{proof}
The $I_k$ are closed intervals in $\R$, so they are compact. The diameter of the sets $I_k$ is the length of the interval $10^{-k}$, which tends to $0$ as $k \rightarrow \infty$. Thus, by the theorems in the book, $\cap_{k=0}^\infty I_k$ is a single point, and we can denote it $p$.

For all $\epsilon > 0$, by the Archimedian property there is some $k \in \N$ such that $\epsilon < 10^{-k}$. Taking the decimal expansion of $p$ and truncating it at $k-1$, $p_0.p_1p_2 \dots p_{k-1}$ corresponds to an interval $I_{k-1} = [p_0.p_1p_2 \dots p_{k-1}, p_0.p_1p_2 \dots (p_{k-1}+1))$ that contains uncountably many elements of $E$. Since 

YOU HAVE THE RIGHT IDEA, YOU JUST NEED TO FIDDLE WITH THE DETAILS TO MAKE SURE THAT THE TRUNCATED P + EPSILON IS IN THE RIGHT INTERVAL.
\end{proof}
\end{theorem}
