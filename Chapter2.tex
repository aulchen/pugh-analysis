...
The bounded, connected, open sets on $\matbb{R}$ are open intervals.
\begin{proof}
...
Let ${A_i}$ be a collection of bounded open intervals of $\mathbb{R}$. Then there exists disjoint open intervals ${B_i}$ such that $\cup A_i = \sqcup B_i$
\begin{proof}
...
\begin{lemma}
\label{DisjointOpenIntervalsAreDisconnected}
If two open intervals in $\mathbb{R}$ are disjoint, then they are disconnected.
\begin{proof}
Let $A$ and $B$ be two disjoint open intervals and let $X = A \cup B$. $A$ is open in $X$ because $A$ is open in $\mathbb{R}$ and $X$ is a metric subset of $\mathbb{R}$. Similarly, $B$ is open in $X$. $A^C$ in $X$ is $B$, so $A$ is closed in $X$. Thus $A$ is clopen in $X$, and since $B$ is nonempty, $A$ is a clopen proper subset of $X$, and thus $X$ is disconnected.
\end{proof}

We will prove the main result via induction. First, the base case.

\begin{lemma}
\label{OpenIntervalIsOpenInterval}
If $A$ is a (possibly unbounded) open interval, then can be uniquely represented by the disjoint union of open intervals $A_i$ such that $A = \sqcup A_i$. Namely, the only element of $A_i$ is $A$ itself.
\begin{proof}
$A_i$ can not be empty because $A$ is not the null set. The size of $A_i$ can not be greater than 1. For if there are two or more elements in $A_i$, $\sqcup A_i$ is disconnected by Lemma \ref{DisjointOpenIntervalsAreDisconnected}, while $A$ is connected. Thus $A_i$ has only one element, and it's obvious that that element is $A$ itself.
\end{proof}

Now, to prove the main theorem. Let $A$ be an open subset of $\mathbb{R}$. By Part a, 
